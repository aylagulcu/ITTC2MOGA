			...
			for (int room = 0; room < parameters.numRooms; room++) {
				for (int time = day * parameters.numDailyPeriods; time < (day + 1)* parameters.numDailyPeriods; time++) {
					...
					if (child2.dataMatrix[room][time] == parameters.UNUSED_EVENT) {
						...
						assign(child2, eventParent, time, room, currentTime,currentRoom);
					} // end if
					else{// put event to that place and find another position for the occupying event:
						int ev2= child2.dataMatrix[room][time];
//						if (ev2!= parameters.UNUSED_EVENT) eventsToMove.add(ev2);

						currentTime= convertionManager.intToTime(child2.Data[eventParent]);
						currentRoom= convertionManager.intToRoom(child2.Data[eventParent]);
						
						if (AssignNewPosition(child2, eventParent, time, room, ev2, currentTime, currentRoom)) // move related data is recorded if it updates the current best move			
			...
			for (int room = 0; room < parameters.numRooms; room++) {
				for (int time = day * parameters.numDailyPeriods; time < (day + 1)* parameters.numDailyPeriods; time++) {
					...
					if (child1.dataMatrix[room][time] == parameters.UNUSED_EVENT) {
						...
						int ev2= child1.dataMatrix[room][time];
//						if (ev2!= parameters.UNUSED_EVENT) eventsToMove.add(ev2);

						currentTime= convertionManager.intToTime(child1.Data[eventParent]);
						currentRoom= convertionManager.intToRoom(child1.Data[eventParent]);
						
						if (AssignNewPosition(child1, eventParent, time, room, ev2, currentTime, currentRoom)) // move related data is recorded if it updates the current best move			
							eventsToMove.add(ev2);
					}
				} // end time for
			} // end room for
...
//		System.out.println("event cannot be assigned due to feasibility: "+ event);
	...
	private boolean AssignNewPosition(Individual child, int event, int newTime, int newRoom, int ev2, int currTime, int currRoom) {
		// here ev2 is definitely not unused event!
		// assigns if new position is feasible
		// it not, takes back all the changes.
		
		// update data matrix:
		child.dataMatrix[newRoom][newTime]= event;
		child.dataMatrix[currRoom][currTime]= ev2;
		...
		for (int cur: curList){
			child.timeCurriculum[newTime][cur]--; // old position
			child.timeCurriculum[currTime][cur]++; // new position
		} // end for each

		int evOrigVal= child.Data[event];
		child.Data[event] = convertionManager.eventValuesToInt(dataHolder.eventCourseId[event], 1, newTime, newRoom);
		int ev2OrigVal= child.Data[ev2];
			child.Data[ev2] = convertionManager.eventValuesToInt(dataHolder.eventCourseId[ev2], 1, currTime, currRoom);
				
		if (checkFeasForMyConstr(child, event, newTime, newRoom) && checkFeasForMyConstr(child, ev2, currTime, currRoom))
			return true; // return the current updated matrices
		
//		System.out.println("event cannot be assigned due to feasibility: "+ event);
		// if not returned true:
		child.Data[event] = evOrigVal; // To original values
		child.Data[ev2] = ev2OrigVal; // To original values
		// matrix to original values:
		child.dataMatrix[currRoom][currTime]= event;
		child.dataMatrix[newRoom][newTime]= ev2;
